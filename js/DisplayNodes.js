...
export function setupDisplayNodes(nodeType, nodeData, app) {
  ...
  } else if (nodeData.name === "Griptape Display: Dictionary") {
    setupDictionaryDisplayNode(nodeType, nodeData, app);
  }
}

function setupArtifactDisplayNode(nodeType, nodeData, app) {
  nodeType.prototype.onNodeCreated = function () {
    this.message = ComfyWidgets.STRING(
      this,
      "Output Text",
...
function setupDictionaryDisplayNode(nodeType, nodeData, app) {
  nodeType.prototype.onExecuted = function (message) {
    console.log("In dictionary display node");
    let stringWidget = this.widgets.find((w) => w.name === "DICTIONARY");
    if (stringWidget && message.hasOwnProperty("INPUT")) {
      let new_val = Array.isArray(message["INPUT"])
        ? message["INPUT"].join("")
        : String(message["INPUT"]);
