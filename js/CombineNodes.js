...
function countSpecialInputs(node) {
    ...
    return node.inputs.filter(input => specialInputs.includes(input.name)).length;
...
function handleInputRemoval(node, index, connected, converted_count) {
    ...
    function isValidRemovalContext(stackTrace) {
        return !stackTrace.includes(CONNECT_TOUCH) &&
...
function renameInputs(node, input_name) {
    ...
    if (
        (last_slot.name == 'select' && last_slot.name != 'sel_mode' && node.inputs[node.inputs.length - 2].link != undefined)
...
function updateWidgets(node, input_name, select_slot) {
    ...
    if (inputWidget) {
        inputWidget.options.max = select_slot ? node.inputs.length - 1 : node.inputs.length;
        inputWidget.value = Math.min(Math.max(1, inputWidget.value), inputWidget.options.max);
    }
}
