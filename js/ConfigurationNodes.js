...
export function setupConfigurationNodes(nodeType, nodeData, app) {
  if (
    ...
    nodeType.prototype.onNodeCreated = async function () {
      ...
      if (engine) {
        if (this.widgets) {
          const modelWidgets = this.widgets.filter((w) =>
            ["model", "prompt_model", "embedding_model", "base_model"].includes(
              w.name
            )
          );
          ...
          const fetchModels = async (engine, baseIp, port) => {
            try {
              ...
              console.error(
                `Error fetching models for engine ${engine}:`,
                error
              );
              return [];
            }
